...
public static boolean LeerAtributos(File xMLPath) {
		...
		try {
			...
			NodeList at = 
					(NodeList)xp.compile("//Atributo").evaluate(d, 
...
public static Nodo VerificarNodoD(String Nombre) {
	...
	if (Grafo.PanelGrafo.nodosD.contains(Nombre)) {
		System.out.println("Si está " + Nombre);
		
		for (int i = 0; i <= Grafo.PanelGrafo.nodos.size();i++) {
			System.out.println("Dentro del Ciclo for " + i + " Nombre buscado = " + Nombre + " Nombre Evaluado = " + Grafo.PanelGrafo.nodos.get(i).getNombre());
...
public static boolean LeerDependencias(File xMLPath) {
	try {
		...
		XPath xp = XPathFactory.newInstance().newXPath();
		NodeList nl = 
				(NodeList)xp.compile("//DF").evaluate(d, 
				XPathConstants.NODESET);
		...
		if (nl.getLength()> 0) { 
			...
			for (int i = 0; i <= nl.getLength(); i++) {
				...
				Nodo dd = new Nodo();
						...
						VentanaPrincipal.PonerTextoDF(xp.compile("./Izq").evaluate(nl.item(i)), xp.compile("./Der").evaluate(nl.item(i)));
